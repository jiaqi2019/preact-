...
export function diffChildren(
	...
	for (i = 0; i < renderResult.length; i++) {
		...
		// (holes).
		oldVNode = oldChildren[i];    //首次,undefined
		...
		if ((j = childVNode.ref) && oldVNode.ref != j) {  //ref ===> {current: null}
			if (!refs) refs = [];
			if (oldVNode.ref) refs.push(oldVNode.ref, null, childVNode);
			refs.push(j, childVNode._component || newDom, childVNode);
			// []数组中,每三个表示一个 ref相关的对象
	...
	if (refs) {
		for (i = 0; i < refs.length; i++) {
			...
			// refs ===> [], 每3个位 一个ref相关的节点和组件
			// line: 161 
			applyRef(refs[i], refs[++i], refs[++i]);
			// ref ==> {current: childVNode._component || newDom}
			// 经过此处, 如果是组件,ref.current就指向了组件
			// 如果是dom节点, ref.current就指向了dom
		}
	}
}
